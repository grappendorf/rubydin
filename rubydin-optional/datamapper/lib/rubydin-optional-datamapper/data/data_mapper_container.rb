	...
	class DataMapperContainer
		...
		class DataMapperItem
			...
			def getItemProperty property_id
				DataMapperProperty.new @item, property_id
			end
		...
		def getContainerPropertyIds
			@model.properties.map(&:name).map(&:to_s)
		end

		def getItem item_id
			DataMapperItem.new @model, item_id
		end

		alias item getItem

		def getItemIds
			@model.all(fields:[:id]).map(&:id)
		end
